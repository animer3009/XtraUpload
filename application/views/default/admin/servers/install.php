...
if($error)
{
	...
	<p></p>
	<p>
		<span class="alert"><b>Error(s) Occured:</b><br /><?=$error?></span>
	</p>
	<?php 
}
else
...
<form action="<?=site_url('/admin/server/install/'.$sid)?>" method="post">
	<input type="hidden" name="step-1" value="true" size="50" />
	<h3>Server FTP Details</h3>
	<p>
		<label>FTP URL</label>
		<input type="text" name="ftp_url" value="<?=($this->input->post('ftp_url') ? $this->input->post('ftp_url') : str_replace('http://','',$server->url))?>" size="50"/><br />
		
		<label>FTP Username</label>
		<input type="text" name="ftp_user" value="<?=$this->input->post('ftp_user')?>" size="20"/><br />
		
		<label>FTP Password</label>
		<input type="password" name="ftp_pass" value="<?=$this->input->post('ftp_pass')?>" size="20"/><br />
		
		<label>FTP Port</label>
		<input type="text" name="ftp_port" value="<?=($this->input->post('ftp_port') ? $this->input->post('ftp_port') : '21')?>" size="7"/><br />
		
		<label>FTP Path to install location</label>
		<input type="text" name="ftp_path" value="<?=$this->input->post('ftp_path')?>" size="50"/><br /><br />
