...
class Install extends CI_Controller
{
	...
	{
		parent::__construct();
        $this->_db_version = include('../../application/xu_ver.php');
	}
    ...
    private function _writeConfig($cookie_prefix, $encryption_key, $index_page, $base_url)
    {
        $config_source = file_get_contents('includes/templates/config/config.php');
        ...
        foreach($config_lines as $key => $value) {
            $pattern = sprintf('#(\$config\[\'%s\'\]\s*=\s*)[\'"][^\'"]*[\'"];#', $key);
            $replacement = sprintf('$1\'%s\';', $value);

            $config_source = preg_replace($pattern, $replacement, $config_source);

            unset($pattern, $replacement);
        }

        $config_modified = trim($config_source) . PHP_EOL;

        file_put_contents('../../application/config/config.php', $config_modified);
    }

    private function _writeDatabase()
    {
        $database_source = file_get_contents('includes/templates/config/database.php');

        $database_lines = array(
            'hostname' => $this->input->post('sql_server'),
            'username' => $this->input->post('sql_user'),
            'password' => $this->input->post('sql_pass'),
            'database' => $this->input->post('sql_name'),
            'dbdriver' => $this->input->post('sql_engine'),
            'dbprefix' => $this->input->post('sql_prefix'),
        );

        foreach($database_lines as $key => $value) {
            $pattern = sprintf('#(\$db\[\'default\'\]\[\'%s\'\]\s*=\s*)\'[^\']*\';#', $key);
            $replacement = sprintf('$1\'%s\';', $value);

            $database_source = preg_replace($pattern, $replacement, $database_source);

            unset($pattern, $replacement);
        }

        $database_modified = trim($database_source) . PHP_EOL;

        file_put_contents('../../application/config/database.php', $database_modified);
        file_put_contents('includes/config/database.php', $database_modified);
    }
...
/* End of install.php */
/* Location: ./setup/includes/controllers/install.php */
